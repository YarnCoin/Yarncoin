...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZYARNModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zyarnTracker->HasMintTx(hash))) {
            //zYARN stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZYARN;
...
bool IsZYARNType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZYARN:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZYARN || type == TransactionRecord::MNReward) {
