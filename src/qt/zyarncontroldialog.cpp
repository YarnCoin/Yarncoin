...
#include "ui_zyarncontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zyarnTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zyarnTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zYARN.";
            else if (!mint.isSeedCorrect)
                strReason = "The zYARN seed used to mint this zYARN is not the same as currently hold in the wallet";
            else
