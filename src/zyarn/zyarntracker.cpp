...
CzYARNTracker::CzYARNTracker(std::string strWalletFile)
{
...
CzYARNTracker::~CzYARNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzYARNTracker::Init()
{
...
bool CzYARNTracker::Archive(CMintMeta& meta)
{
...
bool CzYARNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzYARNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzYARNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzYARNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzYARNTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzYARNTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzYARNTracker::GetSerialHashes()
{
...
CAmount CzYARNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzYARNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzYARNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzYARNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzYARNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzYARNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzYARNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzYARNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzYARNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzYARNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzYARNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzYARNWallet* zYARNWallet)
{
    bool iszYARNWalletInitialized = (NULL != zYARNWallet);
    CMintMeta meta;
    ...
    if (! iszYARNWalletInitialized)
        zYARNWallet = new CzYARNWallet(strWalletFile);
    meta.isSeedCorrect = zYARNWallet->CheckSeed(dMint);
    if (! iszYARNWalletInitialized)
        delete zYARNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzYARNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzYARNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzYARNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzYARNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzYARNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzYARNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzYARNWallet* zYARNWallet = new CzYARNWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zYARNWallet);
        }
        delete zYARNWallet;
        LogPrint("zero", "%s: added %d dzyarn from DB\n", __func__, listDeterministicDB.size());
    }
