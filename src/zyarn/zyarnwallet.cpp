...
CzYARNWallet::CzYARNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zyarn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZYARNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZYARNSeed_deprecated()) {
                    LogPrintf("%s: Updated zYARN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zyarn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zyarn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzYARNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZYARNCount(nCountLastUsed);
    else if (!walletdb.ReadZYARNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzYARNWallet::Lock()
{
    seedMaster = 0;
}

void CzYARNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzYARNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZYARN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzYARNWallet::LoadMintPoolFromDB()
{
...
void CzYARNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzYARNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzYARNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zyarnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzYARNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZYARN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zyarnTracker which also adds to database
    pwalletMain->zyarnTracker->Add(dMint, true);
...
void CzYARNWallet::SeedToZYARN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzYARNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzYARNWallet::GenerateDeterministicZYARN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzYARNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzYARNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzYARNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
