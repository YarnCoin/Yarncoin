...
public:
    ...
    bool WriteZYARNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZYARNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZYARNSeed_deprecated(uint256& seed);
    bool EraseZYARNSeed();
    bool EraseZYARNSeed_deprecated();

    bool WriteZYARNCount(const uint32_t& nCount);
    bool ReadZYARNCount(uint32_t& nCount);
