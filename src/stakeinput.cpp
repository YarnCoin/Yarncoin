...
// The zYARN block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zYARN is a hash of the serial
...
{
    //Create an output returning the zYARN that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZYARNOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zYARN output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zYARN", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZYARNOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zYARN output", __func__);
...
{
    CzYARNTracker* zyarnTracker = pwallet->zyarnTracker.get();
    CMintMeta meta;
    if (!zyarnTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zyarnTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a YARN stake is the outpoint
